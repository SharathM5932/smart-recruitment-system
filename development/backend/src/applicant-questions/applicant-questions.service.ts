...
export class ApplicantQuestionService {
  ...
  async saveAnswer(applicantId: string, attemptId: string, questionId: string, selectedOptionId: string) {
  ...
  async evaluateTest(applicantId: string, attemptId: string) {
    ...
    const correct = answers.filter(a => a.selected_option?.is_correct).length;
